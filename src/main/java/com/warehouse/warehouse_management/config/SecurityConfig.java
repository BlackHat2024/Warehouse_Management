...
import com.warehouse.warehouse_management.filter.JwtAuthenticationFilter;
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.sessionManagement(c->c.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
                .csrf(AbstractHttpConfigurer::disable).authorizeHttpRequests(c->c.requestMatchers("/**").permitAll()
                        .requestMatchers(HttpMethod.POST,"/admin/**").hasRole(Role.SYSTEM_ADMIN.name())
                        .requestMatchers(HttpMethod.POST,"/client/**").hasRole(Role.CLIENT.name())
                        .requestMatchers(HttpMethod.POST,"/manager/**").hasRole(Role.WAREHOUSE_MANAGER.name())
